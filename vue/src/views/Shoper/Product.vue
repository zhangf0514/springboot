  ...
  <div>
    ...
    <el-dialog title="商品信息" :visible.sync="dialogFormVisible" width="30%" >
      <el-form label-width="80px" size="small">
        ...
        <el-select v-if="item!=null" :placeholder="item.itemname"  v-model="form.itemid" clearable  >
         <el-option
          ...
          <el-select v-if="item==null" v-model="form.itemid" clearable  placeholder="请选择"  >
            <el-option
                v-for="item in dropdownmenu"
                :key="item.itemid"
                :label="item.itemname"
...
export default {
  ...
  methods:{
    ...
    save(){
      this.form.shopid=JSON.parse(localStorage.getItem("shopId"))
    ...
    handleEdit(row){
      this.request.get("/item/"+row.itemid)
          .then(res => {
            this.item=res.data
          })
      this.form=row
